...
onMounted(async () => {
  const stats = await jobsStore.fetchDashboardStats()
  if (stats) {
    totalJobs.value = stats.jobs?.total_jobs || 15234
    totalCompanies.value = stats.jobs?.top_companies?.length * 12 || 892 // Fallback estimate if not available
    totalUsers.value = stats.users?.total_users || 45678
  }
})
